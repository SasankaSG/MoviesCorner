...
export const MovieDetail = () => {
  ...
  useTitles(`Search result: ${movie.title}`);

  useEffect(() => {
    async function fetchMovie() {
      const response = await fetch(`https://api.themoviedb.org/3/movie/${params.id}?api_key=c1d09759550e89581de810b0bddbd9e3&language=en-US&page=1`);
        ...
        <div className="max-w-2xl text-gray-700 text-lg dark:text-white">
          ...
          { movie.genres ? <p className="my-7 flex flex-wrap gap-2">
            {movie.genres.map((genre) => (
                <span className="mr-3 border border-gray-200 rounded dark:border-gray-600 p-2" key={genre.id}>{genre.name}</span>
            ))}
          </p> : ""}
